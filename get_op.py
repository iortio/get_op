...
if arguments.repo and arguments.token:
    ...
    if config[0].get('platform', '') == 'python':
        ...
        # Pull image
        os.system('echo "docker pull ghcr.io/%s:main"' % (full_repo_name))
        try: os.system('docker pull ghcr.io/%s:main' % (full_repo_name))
        except: pass
        # Stop container
        os.system('echo "docker stop %s"' % (repo_name))
        try: os.system('docker stop %s' % (repo_name))
        except: pass
        # Remove container
        os.system('echo "docker rm %s"' % (repo_name))
        try: os.system('docker rm %s' % (repo_name))
        except: pass
        # Run image
        try: os.system('echo "docker run -p 0.0.0.0:%d:5001/tcp -d --name %s ghcr.io/%s"' % (config[0].get('port', 5001), repo_name, full_repo_name))
